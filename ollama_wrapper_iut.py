    ...
    def _http_request_json(
        ...
        *,
        ...
        try:
            ...
            # Si le serveur renvoie une erreur (ex: 404 Model not found), on tente de lire le JSON
            try:
                ...
                msg = str(e)
            raise OllamaResponseError(f"Erreur API Ollama ({e.code}): {msg}") from e
...
if __name__ == "__main__":
    ...
    if client.is_server_running():
        ...
        try:
            r = client.generate_text(model="gemma2:latest", prompt="Donne-moi une définition courte de l'IA.", options={"format": "json"})
            print("Generate :", r.response)
        except OllamaResponseError as e:
            try:
                r = client.generate_text(model="gemma3:latest", prompt="Donne-moi une définition courte de l'IA.", options={"format": "json"})
                print("Generate :", r.response)
            except OllamaResponseError as e2:
                print("Erreur génération :", e2)
